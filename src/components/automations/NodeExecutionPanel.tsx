  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <Label className="text-xs font-semibold text-chart-3">Executar Bloco (Unit Test)</Label>
          <p className="text-[10px] text-muted-foreground">
            Executa isoladamente via <code className="bg-muted px-1 rounded font-mono">/v1/create/node/execution</code>
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <Label className="text-xs font-medium text-muted-foreground">Config</Label>
          <Button variant="ghost" size="sm" className="h-7 px-2.5 text-xs gap-1.5" onClick={addField}>
        ...
        <div className="space-y-2.5">
          {fields.map((field, i) => (
            <div key={i} className="space-y-1.5 rounded-lg border border-border p-3 bg-muted/20">
              <div className="flex items-center gap-2">
                <Label className="text-xs font-mono text-muted-foreground flex-1 truncate">{field.key || '(sem nome)'}</Label>
              ...
              {field.type === 'json' ? (
                <Textarea
                  ...
                  placeholder="{}"
                  autoFormatJson
                  className={`text-xs min-h-[70px] font-mono bg-background ${fieldErrors[i] ? 'border-destructive' : ''}`}
                />
              ) : field.type === 'boolean' ? (
                <Select value={field.value.toLowerCase() === 'true' ? 'true' : 'false'} onValueChange={v => updateField(i, { value: v })}>
                  <SelectTrigger className={`h-9 text-sm ${fieldErrors[i] ? 'border-destructive' : ''}`}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Input
                  ...
                  className={`h-9 text-sm font-mono ${fieldErrors[i] ? 'border-destructive' : ''}`}
                />
              )}
          ...
          {fields.length === 0 && (
            <p className="text-xs text-muted-foreground italic py-4 text-center">Nenhum campo disponível.</p>
          )}
        </div>
      </div>
      ...
      >
        {running ? (
          ...
          <><Play className="h-4 w-4" /> Executar bloco</>
        )}
      </Button>
      ...
      {result && (
        <div className="space-y-2.5">
          ...
          <Textarea
            ...
            readOnly
            className={`text-xs min-h-[120px] font-mono ${result.success ? 'bg-chart-2/5 border-chart-2/30' : 'bg-destructive/5 border-destructive/30'}`}
          />
        </div>
      )}
    </div>
